...
func TestTransformToSecurityEvent_FieldMapping(t *testing.T) {
	...
	// Verify finding fields (finding.description was removed from the processor)
	assert.NotEmpty(t, attrs.AsRaw()["finding.id"])
	assert.Equal(t, "medium", attrs.AsRaw()["finding.severity"]) // stored as original value
...
func TestFindingSeverity(t *testing.T) {
	tests := []struct {
		...
		{"low", "low", "low"},
		{"empty", "", "MEDIUM"},                              // Empty severity defaults to MEDIUM
		{"unknown", "unknown", "unknown"},                    // Stored as-is
		{"case sensitive critical", "Critical", "Critical"},  // Stored as-is
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Severity is always set (defaults to "MEDIUM" when empty)
